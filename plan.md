...
The at-index target is the weakest exact boundary for the generated
...
1. **Source reduction.**  Prove one generated position-code row target, ideally
   ...
   `SourcePositionCodeInteriorRowsPrimrec`.  The central reduction and final
   theorem surfaces now accept this target directly via
...
2. **Scaffold instantiation.**  Prove the concrete Figure 13/Figure 16 Section
   ...
   The intermediate compatible-level/layer-patch surface now exposes the same
   source-target variants: interior rows, one rows, bounded rows, decoder step,
