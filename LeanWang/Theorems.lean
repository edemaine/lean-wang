...
/--
A computable reduction from Mathlib TM2 evaluator configurations to finite
table-machine data.

The `Compiler` suffix records how the reduction is implemented: it compiles a
source machine/evaluator instance into a finite `TableProgram`. Mathematically,
the packaged map is the many-one reduction component.
-/
...
/--
Correctness bridge from the code-specific started TM2 evaluator used by the
Mathlib TM0 route back to Mathlib's original `PartrecToTM2.init` evaluator.

The current TM0 route makes the evaluator start label the default label so that
Mathlib's standard `TM2.eval` translation theorem applies. This field records
the remaining semantic equivalence between that started machine and the
original evaluator configuration.
-/
...
def TM0FiniteCompiler.toTM2TableCompiler
  ...
  correct := by
    intro tc
    exact (B.correct (C.compile tc)).trans
      ((C.correct tc).trans
        ((TM0Route.partrecStartedTM0_eval_dom_iff_tm2 tc).trans (R.correct tc)))
...
The `Compiler` suffix emphasizes the implementation side of the reduction: the
map compiles each source code to a finite machine program. In the undecidability
proof, this same computable map is the many-one reduction component from code
...
def TM0FiniteCompiler.toTableCompiler
  ...
  correct := by
    intro c
    exact (B.correct (C.compile (R.translate c))).trans
      ((C.correct (R.translate c)).trans
        ((TM0Route.partrecStartedTM0_eval_dom_iff_tm2 (R.translate c)).trans
          ((R₀.correct (R.translate c)).trans (R.correct c))))
...
/-- Apply the code-to-machine reduction and view the result as a concrete machine. -/
def programMachine (C : TableCompiler) (c : Code) : Machine :=
  (programTable C c).toMachine

/-- Correctness of the reduction from partial-recursive codes to concrete machines. -/
...
table-machine reduction, implemented by compilation to finite machine data.
-/
...
theorem fixed_domino_problem_undecidable_of_tm0Compiler
    ...
    ¬ ComputablePred
      (fun c : Code =>
        TilesQuarterWithSeed
          (fixedDominoReduction (C.toTableCompiler B R₀ R) c).1
          (fixedDominoReduction (C.toTableCompiler B R₀ R) c).2) :=
...
theorem fixed_corner_square_problem_undecidable_of_tm0Compiler
    ...
    ¬ ComputablePred
      (fun c : Code =>
        ∀ n : Nat, 0 < n → TileableFixedCornerSquare
          (fixedDominoReduction (C.toTableCompiler B R₀ R) c).1
          (fixedDominoReduction (C.toTableCompiler B R₀ R) c).2 n) :=
  fixed_corner_square_problem_undecidable (C.toTableCompiler B R₀ R)
...
and a computable reduction from partial-recursive codes to table machines. -/
...
a TM2 table-machine reduction, implemented by compilation to finite machine
data.
-/
...
/--
Unencoded domino undecidability from a scaffold, an encoded TM2 translation,
and a TM2 table-machine reduction, implemented by compilation to finite machine
data.
-/
