...
namespace Closed104
namespace SparseFreeLinePlaneSideHalfProjection
...
open RedCycles RedShadeCycles RedShadeGraph RedShadeGraphRefinement
  ...
  SparseFreeLineSideHalfAudit SparseFreeLinePlaneSideHalfClosure
...
theorem sameComponents_windowAt_shift
    ...
    ∀ x y, x < 24 → y < 16 →
      componentAt
          (iterateRefine 2 (windowGrid (windowAt depth grid blockX))) x y =
        componentAt
          (iterateRefine 2
            (shiftGrid (oldGrid depth grid) (blockX - 1)
...
theorem shiftedRoute_of_windowAt
    ...
    (route : BoundedRouteIn
      (iterateRefine 2 (windowGrid (windowAt depth grid blockX)))
      24 16 starts target) :
    ShiftedBoundedRoute (oldGrid depth grid) (blockX - 1)
      (lowerBlockY depth) 24 16 starts target := by
  have same := sameComponents_windowAt_shift depth grid blockX
  rcases route with ⟨start, hstart, path, targetLive⟩
...
theorem shiftedRouteIn_of_windowAt
    {depth : Nat} {grid : Nat → Nat → Index} {blockX lower upper : Nat}
    {target : Port}
    (route : RouteIn (windowAt depth grid blockX) lower upper target) :
    ShiftedBoundedRoute (oldGrid depth grid) (blockX - 1)
      (lowerBlockY depth) 24 16
      (windowStartsIn (windowAt depth grid blockX) lower upper) target := by
...
theorem rowStart_mem_alignedSelf
    ...
    (hstart : start ∈ rowStarts grid .retained 1) :
    start ∈ alignedRowStarts grid 1 start.port.x := by
...
theorem windowStartsIn_backed
    (depth : Nat) (grid : Nat → Nat → Index) (blockX lower upper : Nat)
    (row : LiveRowCertificate (oldGrid depth grid)
  ...
  have hgrid := windowGrid_windowAt depth grid blockX macroX 1
    hmacroX (by decide)
...
theorem projectsTo_of_windowRoute
    ...
    (hwindowEast : 8 * (blockX - 1) + upper ≤
      quarterEast (4 * east .odd (depth + 1)))
    {target : Port}
    (route : ShiftedBoundedRoute (oldGrid depth grid) (blockX - 1)
      (lowerBlockY depth) 24 16
      (windowStartsIn (windowAt depth grid blockX) lower upper) target) :
  ...
  exact projectsTo_of_shiftedRoute_of_backedStarts
    ...
    (windowStartsIn_backed depth grid blockX lower upper row
      hwindowWest hwindowEast) route

theorem auditedShiftedRoutesIn
    (depth : Nat) (grid : Nat → Nat → Index) (blockX x lower upper
      targetLower targetUpper : Nat)
    (hcheck : windowCheckIn (canonicalWindow (windowAt depth grid blockX))
    ...
    (interior : Signals.verticalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth grid blockX))) (8 + x) 3)
      (quadrantAt (8 + x) 3) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth grid) (blockX - 1)
        (lowerBlockY depth) 24 16
        (windowStartsIn (windowAt depth grid blockX) lower upper)
        ⟨8 + x, 3, .south⟩ ∨
      ShiftedBoundedRoute (oldGrid depth grid) (blockX - 1)
        (lowerBlockY depth) 24 16
        (windowStartsIn (windowAt depth grid blockX) lower upper)
        ⟨8 + x, 3, .north⟩ := by
  let window := windowAt depth grid blockX
  have hgrid : windowGrid (canonicalWindow window) =
      BorderSubstitution.canonicalizeGrid (windowGrid window) := by
    funext gridX gridY
...
theorem auditedLeftmostRoutes
    (depth : Nat) (grid : Nat → Nat → Index) (x : Nat) (hx : x < 8) (htarget : 2 ≤ x)
    (interior : Signals.verticalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth grid (firstBlock depth)))) (8 + x) 3)
      (quadrantAt (8 + x) 3) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth grid) (firstBlock depth - 1)
        (lowerBlockY depth) 24 16
      ...
      ShiftedBoundedRoute (oldGrid depth grid) (firstBlock depth - 1)
        ...
        (windowStartsIn (windowAt depth grid (firstBlock depth)) 10 24)
        ⟨8 + x, 3, .north⟩ :=
  auditedShiftedRoutesIn depth grid (firstBlock depth) x 10 24 2 8
    (leftmostWindows_complete _ (canonical_leftmost_mem depth grid))
    hx htarget hx interior

theorem auditedNextLeftRoutes
    (depth : Nat) (grid : Nat → Nat → Index) (x : Nat) (hx : x < 8)
    (interior : Signals.verticalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth grid (firstBlock depth + 1)))) (8 + x) 3)
      (quadrantAt (8 + x) 3) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth grid) (firstBlock depth + 1 - 1)
        (lowerBlockY depth) 24 16
      ...
      ShiftedBoundedRoute (oldGrid depth grid) (firstBlock depth + 1 - 1)
        ...
        (windowStartsIn (windowAt depth grid (firstBlock depth + 1)) 2 24)
        ⟨8 + x, 3, .north⟩ :=
  auditedShiftedRoutesIn depth grid (firstBlock depth + 1) x 2 24 0 8
    (nextLeftWindows_complete _ (canonical_nextLeft_mem depth grid))
...
theorem auditedRightmostRelevantRoutes
    ...
    (interior : Signals.verticalInterior?
      (componentAt (iterateRefine 2
        ...
        (8 + x) 3)
      (quadrantAt (8 + x) 3) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth grid) (lastRelevantBlock depth - 1)
        (lowerBlockY depth) 24 16
        (windowStartsIn
      ...
      ShiftedBoundedRoute (oldGrid depth grid) (lastRelevantBlock depth - 1)
        ...
        (windowStartsIn
          (windowAt depth grid (lastRelevantBlock depth)) 0 16)
        ⟨8 + x, 3, .north⟩ :=
  auditedShiftedRoutesIn depth grid (lastRelevantBlock depth) x 0 16 0 8
    (rightmostRelevantWindows_complete _
      (canonical_rightmostRelevant_mem depth grid))
    hx (by omega) hx interior
...
theorem auditedShiftedRoutes
    (depth : Nat) (grid : Nat → Nat → Index) (delta x : Nat)
    ...
    (interior : Signals.verticalInterior?
      (componentAt (iterateRefine 2
        ...
        (8 + x) 3)
      (quadrantAt (8 + x) 3) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth grid)
...
theorem verticalProjection_nextOldRow
    ...
    (row : LiveRowCertificate (oldGrid depth grid)
      (west .odd (depth + 1)) (east .odd (depth + 1))
      (west .odd (depth + 1)) (east .odd (depth + 1)) (oldRow depth)) :
    VerticalProjectionAt (oldGrid depth grid)
      (west .odd (depth + 1)) (east .odd (depth + 1))
      (west .odd (depth + 1)) (east .odd (depth + 1))
      (oldRow (depth + 1)) := by
  ...
  have localInterior : Signals.verticalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth grid blockX))) (8 + localX) 3)
    ...
    have hsame := sameComponents_windowAt_shift depth grid blockX
  ...
  · rw [hleftmost] at localInterior htargetX htranslatedX
    ...
    have hwindowEast : 8 * (firstBlock depth - 1) + 24 ≤
      ...
      omega
    rcases auditedLeftmostRoutes depth grid localX hlocalX htargetLower
        localInterior with route | route
    · left
      simpa [translatePort, htargetY, htranslatedX] using
        projectsTo_of_windowRoute depth grid (firstBlock depth) 10 24 row
          hwindowWest hwindowEast route
    · right
      simpa [translatePort, htargetY, htranslatedX] using
        projectsTo_of_windowRoute depth grid (firstBlock depth) 10 24 row
  ...
  · by_cases hnextLeft : blockX = firstBlock depth + 1
    · rw [hnextLeft] at localInterior htargetX htranslatedX
      ...
      have hwindowEast : 8 * (firstBlock depth + 1 - 1) + 24 ≤
        ...
        rw [heastEq, quarterEast]
        omega
      rcases auditedNextLeftRoutes depth grid localX hlocalX localInterior with
        route | route
      · left
    ...
    · by_cases hright : blockX = lastRelevantBlock depth
      · rw [hright] at localInterior htargetX htranslatedX
        ...
        have hwindowEast : 8 * (lastRelevantBlock depth - 1) + 16 ≤
          ...
          omega
        rcases auditedRightmostRelevantRoutes depth grid localX hlocalX
            localInterior with route | route
        · left
          simpa [translatePort, htargetY, htranslatedX] using
      ...
      · let delta := blockX - firstBlock depth
        ...
        have hwindowEast : 8 * (blockX - 1) + 24 ≤
          ...
          omega
        have routes := auditedShiftedRoutes depth grid delta localX hdelta
          hlocalX (by simpa [hblockEq] using localInterior)
        rcases routes with route | route
        · left
          have projected := projectsTo_of_windowRoute depth grid blockX 0 24 row
...
end SparseFreeLinePlaneSideHalfProjection
end Closed104
end Figure13Layers
