...
This adapter is deliberately separate from the local recurrence.  The final
forward theorem depends on the full unbounded free-grid construction, whereas
the finite seam lemmas need only the pair-cover recurrence itself.
-/
...
theorem forcesRoutedFixedCornerSquares_of_step (step : Step) :
  ...
  forcesRoutedFixedCornerSquares
    (lightBoardPairCovers_of_allHolds (allHolds_of_step step))

theorem forcesRoutedFixedCornerSquares_of_containedStep
    (step : ContainedStep) :
    ForcesRoutedFixedCornerSquares ShadedSignals.routedScaffold :=
  forcesRoutedFixedCornerSquares
    (lightBoardPairCovers_of_allHolds (allHolds_of_containedStep step))
