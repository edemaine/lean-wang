...
namespace Closed104
namespace SparseFreeLineSideHalfClosure
...
def windowAt (depth : Nat) (parent : Index) (blockX : Nat) : Window :=
  let originX := blockX - 1
  (List.range 2).flatMap fun y =>
    (List.range 3).map fun x =>
      oldGrid depth parent (originX + x) (lowerBlockY depth + y)
...
theorem windowAt_eq (depth : Nat) (parent : Index) (blockX : Nat) :
    windowAt depth parent blockX =
      [oldGrid depth parent (blockX - 1) (lowerBlockY depth),
       oldGrid depth parent (blockX - 1 + 1) (lowerBlockY depth),
       oldGrid depth parent (blockX - 1 + 2) (lowerBlockY depth),
       oldGrid depth parent (blockX - 1) (lowerBlockY depth + 1),
       oldGrid depth parent (blockX - 1 + 1) (lowerBlockY depth + 1),
       oldGrid depth parent (blockX - 1 + 2) (lowerBlockY depth + 1)] := by
  rfl
...
theorem windowGrid_windowAt (depth : Nat) (parent : Index) (blockX x y : Nat)
    (hx : x < 3) (hy : y < 2) :
    windowGrid (windowAt depth parent blockX) x y =
      oldGrid depth parent (blockX - 1 + x) (lowerBlockY depth + y) := by
...
theorem windowAt_succ (depth : Nat) (parent : Index) (blockX : Nat) :
    windowAt (depth + 1) parent blockX =
      refineWindow
        (windowAt depth parent ((blockX - 1) / 4 + 1))
        ((blockX - 1) % 4) := by
