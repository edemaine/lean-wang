...
namespace Closed104
namespace SparseFreeLineHorizontalSideHalfClosure
...
def windowAt (depth : Nat) (parent : Index) (blockY : Nat) : Window :=
  let originY := blockY - 1
  (List.range 3).flatMap fun y =>
    (List.range 2).map fun x =>
      oldGrid depth parent (lowerBlockX depth + x) (originY + y)

def windowsAt (depth : Nat) (parent : Index) : List Window :=
  windowsIn (oldGrid depth parent)
...
theorem windowAt_eq (depth : Nat) (parent : Index) (blockY : Nat) :
    windowAt depth parent blockY =
      [oldGrid depth parent (lowerBlockX depth) (blockY - 1),
       oldGrid depth parent (lowerBlockX depth + 1) (blockY - 1),
       oldGrid depth parent (lowerBlockX depth) (blockY - 1 + 1),
       oldGrid depth parent (lowerBlockX depth + 1) (blockY - 1 + 1),
       oldGrid depth parent (lowerBlockX depth) (blockY - 1 + 2),
       oldGrid depth parent (lowerBlockX depth + 1) (blockY - 1 + 2)] := by
  rfl
...
theorem windowGrid_windowAt (depth : Nat) (parent : Index) (blockY x y : Nat)
    (hx : x < 2) (hy : y < 3) :
    windowGrid (windowAt depth parent blockY) x y =
      oldGrid depth parent (lowerBlockX depth + x) (blockY - 1 + y) := by
...
theorem windowAt_mem_windowsAt (depth : Nat) (parent : Index) (delta : Nat)
    (hdelta : delta < blockCount depth) :
    windowAt depth parent (firstBlock depth + delta) ∈ windowsAt depth parent := by
  change windowAt depth parent (firstBlock depth + delta) ∈
    (List.range (blockCount depth)).map
      (fun offset => windowAt depth parent (firstBlock depth + offset))
  exact List.mem_map_of_mem (by simpa using hdelta)

/-- Every successor window is a residue slice of a refined predecessor window. -/
theorem windowAt_succ (depth : Nat) (parent : Index) (blockY : Nat) :
    windowAt (depth + 1) parent blockY =
      refineWindow
        (windowAt depth parent ((blockY - 1) / 4 + 1))
        ((blockY - 1) % 4) := by
...
theorem canonicalWindow_windowAt_mem_closedWindows
    ...
    (hdelta : delta < blockCount depth) :
    canonicalWindow (windowAt depth parent (firstBlock depth + delta)) ∈
      closedWindows := by
  ...
  | zero =>
      ...
      exact windowAt_mem_windowsAt 0 parent delta hdelta
  | succ depth ih =>
...
theorem canonical_bottommost_mem (depth : Nat) (parent : Index) :
...
theorem canonical_nextBottom_mem (depth : Nat) (parent : Index) :
...
theorem canonical_topEdge_mem (depth : Nat) (parent : Index) :
...
theorem canonical_topmostRelevant_mem (depth : Nat) (parent : Index) :
  ...
  | succ depth =>
      ...
      exact ⟨canonicalWindow (windowAt depth parent (lastBlock depth)),
        canonical_topEdge_mem depth parent, rfl⟩

end SparseFreeLineHorizontalSideHalfClosure
end Closed104
end Figure13Layers
