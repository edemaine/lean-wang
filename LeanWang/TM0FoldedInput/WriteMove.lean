...
/-!
Local write and move simulation lemmas for the position-coded folded program
with an input-dependent initialization prefix.
-/
...
theorem FoldedConfigRel_input_write_step
    ...
    FoldedConfigRel tc
      ...
      ((positionProgramDataOnInput tc input).nextID id) := by
  ...
  have hprogramStep := positionProgramDataOnInput_step_sim_of_step
    (tc := tc) (input := input) (q := cfg.q) (q' := q') (side := side)
...
set_option linter.flexible false in
theorem FoldedConfigRel_input_move_step
    {tc : Turing.ToPartrec.Code} {input : List SourceSymbol}
    {cfg : Turing.TM0.Cfg SourceSymbol (SourceLabel tc)} {id : PostID}
