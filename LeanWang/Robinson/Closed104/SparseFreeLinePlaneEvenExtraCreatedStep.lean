...
-/
import LeanWang.Robinson.Closed104.SparseFreeLineEvenExtraCreatedPositions
import LeanWang.Robinson.Closed104.SparseFreeLinePlaneEvenExtraCreatedWindowBacking
import LeanWang.Robinson.Closed104.SparseFreeLineEvenExtraWindowCoordinates
...
namespace Closed104
namespace SparseFreeLinePlaneEvenExtraCreatedStep
...
theorem verticalProjection
    ...
    (previous : LiveRowCertificate (oldGrid depth grid)
  ...
  have interior' : Signals.verticalInterior?
      (componentAt (iterateRefine 2 (oldGrid depth grid)) x
  ...
  · have checked' : verticalAncestorAt 0 0
    ...
    · left
      ...
      · simpa [endpoint, sparsePort, oldCoordinate, hcoordinate] using
          (Path.refl (indexGrid := iterateRefine 2 (oldGrid depth grid))
  ...
  · have hcreated : verticalAncestorAt 0 0 cell localX = false :=
    ...
    have localInterior : Signals.verticalInterior?
      ...
      have hcomponent := componentAt_two_block (oldGrid depth grid) 0
        blockX center localX 0 hlocalX (by decide)
      simp only [Nat.zero_add] at hcomponent
      change componentAt (iterateRefine 2 (oldGrid depth grid))
      ...
      rw [show 8 * center = 8 * center + 0 by omega,
        hcomponent, hquadrant] at transported
      exact transported
    have hwindow := canonical_verticalWindowAt_mem depth grid delta hdelta
    rw [← hblock] at hwindow
    have hcenterEq :
        windowGrid (verticalWindowAt depth grid blockX) 1 1 = cell := by
      rw [windowGrid_verticalWindowAt depth grid blockX 1 1
        (by omega) (by omega)]
      dsimp [cell, center]
      congr 2
      omega
    have required : (Signals.verticalInterior?
        (componentAt
          (fineGrid (windowGrid (verticalWindowAt depth grid blockX) 1 1))
    ...
    have created : verticalAncestorAt 0 0
        (windowGrid (verticalWindowAt depth grid blockX) 1 1) localX =
        false := by
      rw [hcenterEq]
      exact hcreated
...
theorem horizontalProjection
    (depth : Nat) (grid : Nat → Nat → Index)
    (previous : LiveColumnCertificate (oldGrid depth grid)
      (west .even (depth + 1)) (east .even (depth + 1))
      (west .even (depth + 1)) (east .even (depth + 1))
      (lineCoordinate .even (depth + 1) (extraChild (pivot depth)))) :
  ...
  have interior' : Signals.horizontalInterior?
      (componentAt (iterateRefine 2 (oldGrid depth grid))
  ...
  · have hcreated : horizontalAncestorAt 0 0 cell localY = false :=
    ...
    have localInterior : Signals.horizontalInterior?
      ...
      rw [show 8 * center = 8 * center + 0 by omega,
        hcomponent, hquadrant] at transported
      exact transported
    have hwindow := canonical_horizontalWindowAt_mem depth grid delta hdelta
    rw [← hblock] at hwindow
    have hcenterEq :
        windowGrid (horizontalWindowAt depth grid blockY) 1 1 = cell := by
      rw [windowGrid_horizontalWindowAt depth grid blockY 1 1
        (by omega) (by omega)]
      dsimp [cell, center]
      congr 2
      omega
    have required : (Signals.horizontalInterior?
        (componentAt
          (fineGrid (windowGrid (horizontalWindowAt depth grid blockY) 1 1))
          0 localY) (quadrantAt 0 localY)).isSome = true := by
      rw [hcenterEq]
      exact Option.isSome_iff_ne_none.mpr localInterior
    have created : horizontalAncestorAt 0 0
        (windowGrid (horizontalWindowAt depth grid blockY) 1 1) localY =
        false := by
      rw [hcenterEq]
      exact hcreated
...
theorem certificates
    ...
    (rows : ∀ offset ∈ offsets (depth + 1),
      LiveRowCertificate (SparseFreeLinePlaneBase.refinedGrid .even (depth + 1) grid)
        (west .even (depth + 1)) (east .even (depth + 1))
        (west .even (depth + 1)) (east .even (depth + 1))
        (lineCoordinate .even (depth + 1) offset))
    (columns : ∀ offset ∈ offsets (depth + 1),
      LiveColumnCertificate (SparseFreeLinePlaneBase.refinedGrid .even (depth + 1) grid)
        (west .even (depth + 1)) (east .even (depth + 1))
        (west .even (depth + 1)) (east .even (depth + 1))
        (lineCoordinate .even (depth + 1) offset)) :
    LiveRowCertificate (SparseFreeLinePlaneBase.refinedGrid .even (depth + 2) grid)
        (west .even (depth + 2)) (east .even (depth + 2))
        (west .even (depth + 2)) (east .even (depth + 2))
        (lineCoordinate .even (depth + 2)
          (mainChild (extraChild (pivot depth)))) ∧
      LiveColumnCertificate (SparseFreeLinePlaneBase.refinedGrid .even (depth + 2) grid)
        (west .even (depth + 2)) (east .even (depth + 2))
        (west .even (depth + 2)) (east .even (depth + 2))
        (lineCoordinate .even (depth + 2)
  ...
  have vertical := verticalProjection depth grid
    (rows (extraChild (pivot depth)) hextra)
  have horizontal := horizontalProjection depth grid
...
end SparseFreeLinePlaneEvenExtraCreatedStep
end Closed104
end Figure13Layers
