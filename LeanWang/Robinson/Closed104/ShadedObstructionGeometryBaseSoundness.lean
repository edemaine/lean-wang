...
-/
import LeanWang.Robinson.Closed104.ShadedObstructionGeometryBaseComplete
import LeanWang.Robinson.Closed104.ShadedFreeLineGraphBase
...
open OrientedRedCycles RedShadeGraph RedShadeGraphSearch RedShadeGraphSearchSoundness
  ...
  ShadedFreeLineGraphBase ShadedObstructionGeometry
  ShadedObstructionGeometryBaseAudit ShadedPlaneSignalGrid Signals.FreeCellLocal
...
private theorem horizontal_not_reached_same
    ...
    (shaded : CycleShade stateGrid 4 12 4 12
      (if parentLight then .light else .dark))
    {column row : Nat} (hcolumnBound : column < 32) {side : Side}
...
theorem mem_coordinates_iff {coordinate : Nat} :
    coordinate ∈ coordinates ↔ 9 < coordinate ∧ coordinate < 24 := by
  simp only [coordinates, List.mem_map, List.mem_range]
  constructor
  · rintro ⟨offset, hoffset, rfl⟩
    omega
  · intro hbounds
    exact ⟨coordinate - 10, by omega, by omega⟩
...
theorem freeRow_eq_true_iff
    ...
    freeRow parent (reachedBitmap (nodes parent)) parentLight row = true ↔
      IsFreeRow (localGrid parent) stateGrid 4 12 row := by
  constructor
...
theorem freeColumn_eq_true_iff
    ...
    freeColumn parent (reachedBitmap (nodes parent)) parentLight column = true ↔
      IsFreeColumn (localGrid parent) stateGrid 4 12 column := by
  constructor
...
theorem geometry_of_audit
    ...
    (complete : completeFor parent (reachedBitmap (nodes parent))
      parentLight = true) :
    Geometry (localGrid parent) stateGrid 4 12 4 12 := by
  constructor
...
theorem geometry_canonical
    (parent : Index)
    {stateGrid : Nat → Nat → RedShades.State}
    (valid : ValidShadeGrid
      (localGrid (BorderSubstitution.canonicalIndex parent)) stateGrid)
    (parentLight : Bool)
    (cycle : CycleOn
      (localGrid (BorderSubstitution.canonicalIndex parent)) 4 12 4 12)
    (shaded : CycleShade stateGrid 4 12 4 12
      (if parentLight then .light else .dark)) :
    Geometry (localGrid (BorderSubstitution.canonicalIndex parent))
      stateGrid 4 12 4 12 :=
  geometry_of_audit (BorderSubstitution.canonicalIndex parent) valid parentLight
    cycle shaded
