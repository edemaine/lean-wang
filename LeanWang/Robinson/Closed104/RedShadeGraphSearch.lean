...
/-- Independently check the path retained by a search node. Search is
untrusted; only nodes accepted by this verifier are consumed by proofs. -/
def Node.valid (indexGrid : Nat → Nat → Index)
    (width height : Nat) (node : Node) : Bool :=
  decide (boundedEndpoint indexGrid width height node.origin
    node.reverseMoves.reverse = some (node.current, node.parity))

theorem Node.boundedPath_of_valid
    {indexGrid : Nat → Nat → Index} {width height : Nat} {node : Node}
    (valid : node.valid indexGrid width height = true) :
    RedShadeGraphBoundedPath.BoundedPath indexGrid width height
      node.origin node.current node.parity := by
  simp only [Node.valid, decide_eq_true_eq] at valid
  exact boundedPath_of_boundedEndpoint valid
