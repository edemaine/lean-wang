...
The finite graph search never leaves its `16 x 16` window.  Consequently its
...
namespace Closed104
namespace ShadedObstructionGeometryOddBaseBoundedSoundness
...
open OrientedRedCycles RedCycles RedShadeGraph RedShadeGraphSearch
  ...
  RedShadeGraphRefinement RedShadePaths ShadedFreeLineOddBase
  ShadedObstructionGeometryOddBaseAudit
  ShadedObstructionGeometryOddBaseSoundness ShadedObstructionGeometry
...
theorem reachedWithParity_has_bounded_path
    ...
    (hreached : reachedWithParity parent (reachedBitmap (nodes parent))
      port parity = true) :
    ∃ start ∈ boardPorts,
      BoundedPath (localGrid parent) 16 16 start port parity := by
  rcases ShadedObstructionGeometryOddBaseSoundness.reachedWithParity_has_node
...
theorem selectedHorizontal_semantic
  ...
  have hcolumnBound :=
    (ShadedObstructionGeometryOddBaseSoundness.mem_coordinates_iff.1 hcolumn).2.trans
      (by decide : 12 < 16)
  have hrowBound :=
    (ShadedObstructionGeometryOddBaseSoundness.mem_coordinates_iff.1 hrow).2.trans
      (by decide : 12 < 16)
  have hparts :
...
private theorem horizontal_not_reached_same
    ...
    (shaded : CycleShade stateGrid 2 6 2 6
      (if parentLight then .light else .dark))
    {column row : Nat} (hcolumnBound : column < 16)
    (hrowBound : row < 16) {side : Side}
...
theorem freeRow_eq_true_iff
    ...
    freeRow parent (reachedBitmap (nodes parent)) parentLight row = true ↔
      IsFreeRow targetGrid stateGrid 2 6 row := by
  constructor
...
theorem freeColumn_eq_true_iff
    ...
    freeColumn parent (reachedBitmap (nodes parent)) parentLight column = true ↔
      IsFreeColumn targetGrid stateGrid 2 6 column := by
  constructor
  ...
  · exact selectedHorizontal_eq_of_interior
      ...
      (by
        rw [← componentsEq column boundary
          ((mem_coordinates_iff.1 hcolumn).2.trans (by decide : 12 < 16))
          ((mem_coordinates_iff.1 hboundary).2.trans (by decide : 12 < 16))]
        exact hparts.1.1.2)
  · intro y hry hyb
  ...
  · exact selectedVertical_eq_of_interior
      ...
      (by
        rw [← componentsEq boundary row
          ((mem_coordinates_iff.1 hboundary).2.trans (by decide : 12 < 16))
          ((mem_coordinates_iff.1 hrow).2.trans (by decide : 12 < 16))]
        exact hparts.1.1.2)
  · intro x hcx hxb
...
theorem geometry_of_audit
    ...
    (complete : completeFor parent (reachedBitmap (nodes parent))
      parentLight = true) :
    ShadedObstructionGeometry.Geometry targetGrid stateGrid 2 6 2 6 := by
  constructor
...
theorem geometry
    ...
    (shaded : CycleShade stateGrid 2 6 2 6
      (if parentLight then .light else .dark)) :
    ShadedObstructionGeometry.Geometry targetGrid stateGrid 2 6 2 6 :=
  geometry_of_audit parent componentsEq valid
...
theorem geometry_shift
    (grid : Nat → Nat → Index)
    {stateGrid : Nat → Nat → RedShades.State}
    (valid : ValidShadeGrid (iterateRefine 3 grid) stateGrid)
    (blockX blockY : Nat) :
    Geometry
      (iterateRefine 3 (RefinementTranslation.shiftGrid grid blockX blockY))
      (ShadedFreeLineTranslation.shiftQuarterGrid stateGrid
        (16 * blockX) (16 * blockY)) 2 6 2 6 := by
  ...
  have localValid : ValidShadeGrid targetGrid targetState := by
    simpa only [targetGrid, targetState, show 2 ^ (3 + 1) = 16 by norm_num] using
      ShadedFreeLineTranslation.validShadeGrid_shift 3 grid valid blockX blockY
  have localCycle : CycleOn targetGrid 2 6 2 6 := by
    have cycle := OrientedRedBoardTranslations.at_scale
      (RefinementTranslation.shiftGrid grid blockX blockY) 1 0 0
    norm_num at cycle
  ...
  have componentsEq : ∀ x y, x < 16 → y < 16 →
    ...
    intro x y hx hy
    exact componentAt_localGrid_eq_shift grid blockX blockY rfl hx hy
...
end ShadedObstructionGeometryOddBaseBoundedSoundness
end Closed104
end Figure13Layers
