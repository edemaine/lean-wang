...
theorem moveRight_corePrefix
  ...
  refine ⟨hinstall, hcore, hnextCore, ?_⟩
...
theorem machine_reaches_incrementSchedule_or_halts_of_prefix
  ...
  let E : IncrementEnvelope growth limit := {
    Represents := fun current U => CorePrefixRepresents current growth limit U
    core_before_limit := fun h => h.core_before_limit
    increment := by
      intro current U r h hroom
      exact incrementCoreTape_preserves_prefix h r hroom
    clock := by
      intro base c source searchSlot success collision current U h hroom hraw
      exact machine_reaches_incrementClock_or_halts_of_prefix base c source
        searchSlot success collision h hroom hraw
    internal := by
      ...
      exact machine_reaches_incrementInternal_or_halts_of_prefix base c limit
        counterState searchSlot success collision h i hpositive hdistance
        hlower hupper hsameEnd hmove hraw }
