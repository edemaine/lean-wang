...
theorem machine_reaches_decrementClockSchedule_or_halts_of_prefix
    (base : Nat) (c : Nat.Partrec.Code) (source : Nat)
    {registers : Registers} {growth : Turing.Dir} {limit : Nat}
    {T : FullTM0.Tape (Symbol numTags)}
    (h : CorePrefixRepresents registers growth limit T)
    ...
    (FullTM0.Reaches (CounterControlNestingBridge.machine base c)
        ...
        ⟨resolve base c (directRef growth source finishDirectSlot),
          atLogical growth
            ...
            (layoutEnd registers)⟩ ∧
      CorePrefixRepresents (registers.decrement .clock) growth limit
        (decrementCoreTape registers growth .clock T)) ∨
      FullTM0.HaltsFrom (CounterControlNestingBridge.machine base c)
        ⟨searchState base c ⟨growth, source, secondarySearchBase⟩,
          atLogical growth T (layoutEnd registers)⟩ := by
  ...
  have hmove : MarkerMachine.moveAt .left
    ...
    rw [hinv] at hm
    exact hm
  have hrun := machine_reaches_decrementFirst_or_halts_of_prefix
    (next := next) base c limit source secondarySearchBase
    (directRef growth source finishDirectSlot)
    (by have hcore := h.core_before_limit; omega) h
...
theorem machine_reaches_decrementTempSchedule_or_halts_of_prefix
    (base : Nat) (c : Nat.Partrec.Code) (source : Nat)
    {registers : Registers} {growth : Turing.Dir} {limit : Nat}
    {T : FullTM0.Tape (Symbol numTags)}
    (h : CorePrefixRepresents registers growth limit T)
    ...
    (FullTM0.Reaches (CounterControlNestingBridge.machine base c)
        ...
        ⟨resolve base c (directRef growth source finishDirectSlot),
          atLogical growth
            ...
            (layoutEnd registers)⟩ ∧
      CorePrefixRepresents (registers.decrement .temp) growth limit
        (decrementCoreTape registers growth .temp T)) ∨
      FullTM0.HaltsFrom (CounterControlNestingBridge.machine base c)
        ⟨searchState base c ⟨growth, source, secondarySearchBase⟩,
  ...
  have hclockPrefix : CorePrefixRepresents clockRegs growth limit clockTape := by
    apply installCore_prefix_of_extension h
    · rw [hclockEnd]
    · rw [hclockEnd]
      exact h.core_before_limit
  ...
  have hthree := machine_reaches_decrementFirst_or_halts_of_prefix
  ...
  have hrawFour : RawCommand.markerShift
    ...
    simp [decrementShiftCommands, decrementShiftCommandsAux,
      MarkerShift.decrementOrder]
  have hfour := machine_reaches_decrementFollowing_or_halts_of_prefix
    (next := final) base c limit source (secondarySearchBase + 1)
    (directRef growth source finishDirectSlot) hclockPrefix (3 : Fin 4)
    (by simp [clockRegs, final, RegisterLayout.values,
      Registers.increment, Registers.decrement, Registers.set,
      Registers.get])
    (registerValue_lt_limit_of_prefix hclockPrefix (3 : Fin 4))
  ...
  · exact Or.inl ⟨by simpa [hhead] using hrun,
        decrementCoreTape_preserves_prefix h .temp hpositive⟩
  · exact Or.inr (by simpa [hhead] using hhalts)

/-- The three-shift right-decrement schedule resolves on an open core. -/
theorem machine_reaches_decrementRightSchedule_or_halts_of_prefix
    (base : Nat) (c : Nat.Partrec.Code) (source : Nat)
    {registers : Registers} {growth : Turing.Dir} {limit : Nat}
    {T : FullTM0.Tape (Symbol numTags)}
    (h : CorePrefixRepresents registers growth limit T)
    ...
    (FullTM0.Reaches (CounterControlNestingBridge.machine base c)
        ...
        ⟨resolve base c (directRef growth source finishDirectSlot),
          atLogical growth
            ...
            (layoutEnd registers)⟩ ∧
      CorePrefixRepresents (registers.decrement .right) growth limit
        (decrementCoreTape registers growth .right T)) ∨
      FullTM0.HaltsFrom (CounterControlNestingBridge.machine base c)
        ⟨searchState base c ⟨growth, source, secondarySearchBase⟩,
  ...
  have hclockEnd : layoutEnd clockRegs = layoutEnd registers := by
    rw [← hinv]
    simp only [clockRegs, layoutEnd_increment]
  have htempPrefix : CorePrefixRepresents tempRegs growth limit tempTape := by
    apply installCore_prefix_of_extension h
    · rw [htempEnd]
    · rw [htempEnd]
      exact h.core_before_limit
  ...
  have hrawTwo : RawCommand.markerShift
    ...
    simp [decrementShiftCommands, decrementShiftCommandsAux,
      MarkerShift.decrementOrder]
  have htwo := machine_reaches_decrementFirst_or_halts_of_prefix
  ...
  have hrawThree : RawCommand.markerShift
    ...
    simp [decrementShiftCommands, decrementShiftCommandsAux,
      MarkerShift.decrementOrder]
  have hthree := machine_reaches_decrementFollowing_or_halts_of_prefix
    (next := clockRegs) base c limit source (secondarySearchBase + 1)
    (searchRef growth source (secondarySearchBase + 2)) htempPrefix (2 : Fin 4)
    (by simp [tempRegs, final, RegisterLayout.values,
      Registers.increment, Registers.decrement, Registers.set,
      Registers.get])
    (registerValue_lt_limit_of_prefix htempPrefix (2 : Fin 4))
  ...
  · exact Or.inl ⟨by simpa [hheadTwo] using hrun,
        decrementCoreTape_preserves_prefix h .right hpositive⟩
  · exact Or.inr (by simpa [hheadTwo] using hhalts)

/-- The four-shift left-decrement schedule resolves on an open core. -/
theorem machine_reaches_decrementLeftSchedule_or_halts_of_prefix
    (base : Nat) (c : Nat.Partrec.Code) (source : Nat)
    {registers : Registers} {growth : Turing.Dir} {limit : Nat}
    {T : FullTM0.Tape (Symbol numTags)}
    (h : CorePrefixRepresents registers growth limit T)
    ...
    (FullTM0.Reaches (CounterControlNestingBridge.machine base c)
        ...
        ⟨resolve base c (directRef growth source finishDirectSlot),
          atLogical growth
            ...
            (layoutEnd registers)⟩ ∧
      CorePrefixRepresents (registers.decrement .left) growth limit
        (decrementCoreTape registers growth .left T)) ∨
      FullTM0.HaltsFrom (CounterControlNestingBridge.machine base c)
        ⟨searchState base c ⟨growth, source, secondarySearchBase⟩,
  ...
  have hclockEnd : layoutEnd clockRegs = layoutEnd registers := by
    rw [← hinv]
    simp only [clockRegs, layoutEnd_increment]
  have hrightPrefix : CorePrefixRepresents rightRegs growth limit rightTape := by
    apply installCore_prefix_of_extension h
    · rw [hrightEnd]
    · rw [hrightEnd]
      exact h.core_before_limit
  ...
  have hrawOne : RawCommand.markerShift
    ...
    simp [decrementShiftCommands, decrementShiftCommandsAux,
      MarkerShift.decrementOrder]
  have hone := machine_reaches_decrementFirst_or_halts_of_prefix
  ...
  have hrawTwo : RawCommand.markerShift
    ...
    simp [decrementShiftCommands, decrementShiftCommandsAux,
      MarkerShift.decrementOrder]
  have htwo := machine_reaches_decrementFollowing_or_halts_of_prefix
    (next := tempRegs) base c limit source (secondarySearchBase + 1)
    (searchRef growth source (secondarySearchBase + 2)) hrightPrefix (1 : Fin 4)
    (by simp [rightRegs, final, RegisterLayout.values,
      Registers.increment, Registers.decrement, Registers.set,
      Registers.get])
    (registerValue_lt_limit_of_prefix hrightPrefix (1 : Fin 4))
  ...
  have hthree := machine_reaches_decrementFollowing_or_halts_of_prefix
    ...
    (searchRef growth source (secondarySearchBase + 3)) htempPrefix (2 : Fin 4)
    (by simp [tempRegs, final, RegisterLayout.values,
      Registers.increment, Registers.decrement, Registers.set,
      Registers.get])
    (registerValue_lt_limit_of_prefix htempPrefix (2 : Fin 4))
  ...
  · exact Or.inl ⟨by simpa [hheadOne] using hrun,
        decrementCoreTape_preserves_prefix h .left hpositive⟩
  ...
  | clock =>
      have hraw : RawCommand.markerShift
        ...
        simp [decrementShiftCommands, decrementShiftCommandsAux,
          MarkerShift.decrementOrder]
      simpa [MarkerSchedule.decrementStartBoundary] using
        machine_reaches_decrementClockSchedule_or_halts_of_prefix base c source
          h hpositive hraw
  | temp =>
      simpa [MarkerSchedule.decrementStartBoundary] using
        machine_reaches_decrementTempSchedule_or_halts_of_prefix base c source
          h hpositive hcommands
  | right =>
      simpa [MarkerSchedule.decrementStartBoundary] using
        machine_reaches_decrementRightSchedule_or_halts_of_prefix base c source
          h hpositive hcommands
  | left =>
      simpa [MarkerSchedule.decrementStartBoundary] using
        machine_reaches_decrementLeftSchedule_or_halts_of_prefix base c source
          h hpositive hcommands

/-! ## Complete validated decrement bodies -/

/-- The zero branch resolves from the validated body entry without changing
