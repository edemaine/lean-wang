...
This is definitionally the semantic machine construction, but gives the
reduction side a data-level surface to target before unpacking into `Machine`.
-/
...
in terms of the finite table data consumed by the current tile backend.
-/
...
theorem mem_tableProgramTilesData_iff (P : TableProgram) (tile : WangTile) :
    tile ∈ tableProgramTilesData P ↔ tile ∈ tableProgramTiles P := by
  unfold tableProgramTilesData tableProgramTiles machineTiles taggedMachineTiles
  simp [mem_tableProgramInitialRowMachineTilesData_iff,
    mem_tableProgramNormalRowMachineTilesData_iff]
...
theorem tableProgramFixedDominoData_mem_iff
    (P : TableProgram) (tile : WangTile) :
    tile ∈ (tableProgramFixedDominoData P).1 ↔
      tile ∈ (tableProgramFixedDomino P).1 := by
  simp [tableProgramFixedDominoData, tableProgramFixedDomino,
    mem_tableProgramTilesData_iff]

theorem tableProgramFixedDominoData_seed_eq
    (P : TableProgram) :
    (tableProgramFixedDominoData P).2 = (tableProgramFixedDomino P).2 := by
  simp [tableProgramFixedDominoData, tableProgramFixedDomino,
    tableProgramSeedData_eq_tableProgramSeed]
