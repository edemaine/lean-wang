...
namespace Closed104
namespace SparseFreeLineHorizontalSideHalfProjection
...
open RedCycles RedShadeCycles RedShadeGraph RedShadeGraphRefinement
  ...
  SparseFreeLineHorizontalSideHalfClosure
...
theorem sameComponents_windowAt_shift
    ...
    ∀ x y, x < 16 → y < 24 →
      componentAt
          (iterateRefine 2 (windowGrid (windowAt depth parent blockY))) x y =
        componentAt
          (iterateRefine 2
            (shiftGrid (oldGrid depth parent) (lowerBlockX depth)
              (blockY - 1))) x y := by
  intro x y hx hy
  apply SparseFreeLineSideHalfProjection.componentAt_iterateRefine_two_congr_at
...
theorem shiftedRoute_of_windowAt
    ...
    (route : BoundedRouteIn
      (iterateRefine 2 (windowGrid (windowAt depth parent blockY)))
      16 24 starts target) :
    ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
      (blockY - 1) 16 24 starts target := by
  have same := sameComponents_windowAt_shift depth parent blockY
  rcases route with ⟨start, hstart, path, targetLive⟩
...
theorem shiftedRouteIn_of_windowAt
    {depth : Nat} {parent : Index} {blockY lower upper : Nat}
    {target : Port}
    (route : RouteIn (windowAt depth parent blockY) lower upper target) :
    ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
      (blockY - 1) 16 24
      (windowStartsIn (windowAt depth parent blockY) lower upper) target := by
  exact shiftedRoute_of_windowAt route

theorem columnStart_mem_alignedSelf
    {parent : Index} {start : WeightedStart}
    (hstart : start ∈ columnStarts parent .retained 1) :
    start ∈ alignedColumnStarts parent 1 start.port.y := by
...
theorem windowStartsIn_backed
    (depth : Nat) (parent : Index) (blockY lower upper : Nat)
    (column : LiveColumnCertificate (oldGrid depth parent)
  ...
  have hgrid := windowGrid_windowAt depth parent blockY 1 macroY
    (by decide) hmacroY
...
theorem projectsTo_of_windowRoute
    ...
    (hwindowNorth : 8 * (blockY - 1) + upper ≤
      quarterNorth (4 * east .odd (depth + 1)))
    {target : Port}
    (route : ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
      (blockY - 1) 16 24
      (windowStartsIn (windowAt depth parent blockY) lower upper) target) :
    Nonempty (ProjectsTo (grid := oldGrid depth parent)
      (west := west .odd (depth + 1)) (east := east .odd (depth + 1))
      (south := west .odd (depth + 1)) (north := east .odd (depth + 1))
      (translatePort target (8 * lowerBlockX depth)
        (8 * (blockY - 1)))) := by
  exact SparseFreeLineSideHalfProjection.projectsTo_of_shiftedRoute_of_backedStarts
    (lowerBlockX depth) (blockY - 1) 16 24
    (windowStartsIn_backed depth parent blockY lower upper column
      hwindowSouth hwindowNorth) route

theorem auditedShiftedRoutesIn
    (depth : Nat) (parent : Index) (blockY y lower upper
      targetLower targetUpper : Nat)
    (hcheck : windowCheckIn (canonicalWindow (windowAt depth parent blockY))
    ...
    (interior : Signals.horizontalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth parent blockY))) 3 (8 + y))
      (quadrantAt 3 (8 + y)) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
        (blockY - 1) 16 24
        (windowStartsIn (windowAt depth parent blockY) lower upper)
        ⟨3, 8 + y, .west⟩ ∨
      ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
        (blockY - 1) 16 24
        (windowStartsIn (windowAt depth parent blockY) lower upper)
        ⟨3, 8 + y, .east⟩ := by
  let window := windowAt depth parent blockY
  have hgrid : windowGrid (canonicalWindow window) =
      BorderSubstitution.canonicalizeGrid (windowGrid window) := by
    funext gridX gridY
...
theorem auditedBottommostRoutes
    (depth : Nat) (parent : Index) (y : Nat) (hy : y < 8) (htarget : 2 ≤ y)
    (interior : Signals.horizontalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth parent (firstBlock depth)))) 3 (8 + y))
      (quadrantAt 3 (8 + y)) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
      ...
      ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
        ...
        (windowStartsIn (windowAt depth parent (firstBlock depth)) 10 24)
        ⟨3, 8 + y, .east⟩ :=
  auditedShiftedRoutesIn depth parent (firstBlock depth) y 10 24 2 8
    (bottommostWindows_complete _ (canonical_bottommost_mem depth parent))
    hy htarget hy interior

theorem auditedNextBottomRoutes
    (depth : Nat) (parent : Index) (y : Nat) (hy : y < 8)
    (interior : Signals.horizontalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth parent (firstBlock depth + 1)))) 3 (8 + y))
      (quadrantAt 3 (8 + y)) ≠ none) :
    ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
      ...
      ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
        ...
        (windowStartsIn (windowAt depth parent (firstBlock depth + 1)) 2 24)
        ⟨3, 8 + y, .east⟩ :=
  auditedShiftedRoutesIn depth parent (firstBlock depth + 1) y 2 24 0 8
    (nextBottomWindows_complete _ (canonical_nextBottom_mem depth parent))
    ...
    ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
      ...
      ShiftedBoundedRoute (oldGrid depth parent) (lowerBlockX depth)
        ...
        (windowStartsIn
          (windowAt depth parent (lastRelevantBlock depth)) 0 16)
        ⟨3, 8 + y, .east⟩ :=
  auditedShiftedRoutesIn depth parent (lastRelevantBlock depth) y 0 16 0 8
    (topmostRelevantWindows_complete _
      (canonical_topmostRelevant_mem depth parent))
    hy (by omega) hy interior
...
theorem auditedShiftedRoutes
    (depth : Nat) (parent : Index) (delta y : Nat)
...
theorem horizontalProjection_nextOldColumn
  ...
  have localInterior : Signals.horizontalInterior?
      (componentAt (iterateRefine 2
        (windowGrid (windowAt depth parent blockY))) 3 (8 + localY))
    ...
    have hsame := sameComponents_windowAt_shift depth parent blockY
  ...
  · rw [hbottommost] at localInterior htargetY htranslatedY
    ...
    have hwindowNorth : 8 * (firstBlock depth - 1) + 24 ≤
      ...
      omega
    rcases auditedBottommostRoutes depth parent localY hlocalY htargetLower
        localInterior with route | route
    · left
      simpa [translatePort, htargetX, htranslatedY] using
        projectsTo_of_windowRoute depth parent (firstBlock depth) 10 24 column
          hwindowSouth hwindowNorth route
    · right
      simpa [translatePort, htargetX, htranslatedY] using
        projectsTo_of_windowRoute depth parent (firstBlock depth) 10 24 column
  ...
  · by_cases hnextBottom : blockY = firstBlock depth + 1
    · rw [hnextBottom] at localInterior htargetY htranslatedY
      ...
      have hwindowNorth : 8 * (firstBlock depth + 1 - 1) + 24 ≤
        ...
        rw [hnorthEq, quarterNorth]
        omega
      rcases auditedNextBottomRoutes depth parent localY hlocalY localInterior with
        route | route
      · left
    ...
    · by_cases htop : blockY = lastRelevantBlock depth
      · rw [htop] at localInterior htargetY htranslatedY
        ...
        have hwindowNorth : 8 * (lastRelevantBlock depth - 1) + 16 ≤
          ...
          omega
        rcases auditedTopmostRelevantRoutes depth parent localY hlocalY
            localInterior with route | route
        · left
          simpa [translatePort, htargetX, htranslatedY] using
      ...
      · let delta := blockY - firstBlock depth
        ...
        have hwindowNorth : 8 * (blockY - 1) + 24 ≤
          ...
          omega
        have routes := auditedShiftedRoutes depth parent delta localY hdelta
          hlocalY (by simpa [hblockEq] using localInterior)
        rcases routes with route | route
        · left
          have projected := projectsTo_of_windowRoute depth parent blockY 0 24
...
end SparseFreeLineHorizontalSideHalfProjection
end Closed104
end Figure13Layers
