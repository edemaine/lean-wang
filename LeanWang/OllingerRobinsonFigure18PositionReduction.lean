...
compatible-square package, with `positionProgramData` semantic correctness
discharged.
-/
theorem
    encoded_domino_problem_undecidable_l2c1_robinson_section7_compatible_data_interiorRowsCorrect
...
-/
theorem
    domino_problem_undecidable_l2c1_robinson_section7_compatible_data_interiorRowsCorrect
    (data : L2C1RobinsonSection7CompatibleData)
    (hinterior : Primrec (fun p : Code × Nat × Nat × TM0Route.PartrecVar =>
      sourcePositionCodeInteriorRowsIndexVar p.1 p.2.1 p.2.2.1 p.2.2.2)) :
    ¬ ComputablePred (fun T : TileSet => TilesPlane T) := by
...
-/
theorem
    encoded_domino_problem_undecidable_l2c2_robinson_section7_compatible_data_interiorRowsCorrect
...
-/
theorem
    domino_problem_undecidable_l2c2_robinson_section7_compatible_data_interiorRowsCorrect
    (data : L2C2RobinsonSection7CompatibleData)
    (hinterior : Primrec (fun p : Code × Nat × Nat × TM0Route.PartrecVar =>
      sourcePositionCodeInteriorRowsIndexVar p.1 p.2.1 p.2.2.1 p.2.2.2)) :
    ¬ ComputablePred (fun T : TileSet => TilesPlane T) := by
